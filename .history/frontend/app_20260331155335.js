...
const API_URL = 'http://localhost/smartparking/backend/api_status.php';
...
function buatKartu(slot) {
 ...
 const kelas = kosong ? "kosong" : (slot.status === "terisi" ? "terisi" :
"unknown");
...
function updateTampilan(data) {
 ...
 document.getElementById("totalTerisi").textContent = data.total_slot -
data.total_kosong;
 ...
 const el = existing[i];
 const kosong = slot.status === "kosong";
 el.className = `sp-slot ${kosong ? "kosong" : (slot.status === "terisi"
? "terisi" : "unknown")}`;
 el.querySelector(".sp-slot-ikon").textContent = kosong ? "🟢" :
(slot.status === "terisi" ? "🔴" : "⚪");
 el.querySelector(".sp-slot-status").textContent = kosong ? "TERSEDIA" :
(slot.status === "terisi" ? "TERISI" : "—");
 el.querySelector(".sp-slot-jarak").textContent = slot.jarak.toFixed(1)
+ " cm";
 });
 } else {
 ...
 grid.innerHTML = '<div class="sp-loading">Menunggu data dari
sensor...</div>';
 } else {
